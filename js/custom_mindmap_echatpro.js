    ...
    function a(t) {
        ...
        var r = d3.scaleLinear().domain([0, 180, 360]).range([1, .3, 1]),
            ...
            f = u.append("g").attr("class", "node").attr("transform", function (e) {
                ...
                d3.select(this).select("text").transition(600).style("fill-opacity", 1)
            });
        f.append("circle").attr("r", 0).style("fill", function (t) {
            return t.color = t.data.color;
        ...
        m.select("circle").style("opacity", 0).attr("r", 0), m.select("text").style("fill-opacity", 1);
